...
class SideCoordinateTests(unittest.TestCase):
    def assert_mapping(self, side, expected_point, expected_depth):
        point, depth = panel_to_side_coordinates(side, 5, 20, -7, 100, 80)
    ...
    def test_public_side_mappings(self):
        ...
        self.assert_mapping(MachiningSide.SIDE3, Vec2(5, -7), -20)
        self.assert_mapping(MachiningSide.SIDE5, Vec2(5, -7), -60)
        self.assert_mapping(MachiningSide.SIDE4, Vec2(20, -7), -95)
        self.assert_mapping(MachiningSide.SIDE6, Vec2(20, -7), -5)

    def test_side6_five_mm_inward_has_minus_five_depth(self):
        _, depth = panel_to_side_coordinates(
            MachiningSide.SIDE6, 5, 30, -4, 100, 80,
        )
